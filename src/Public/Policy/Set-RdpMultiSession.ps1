...
.EXAMPLE
PS C:\> Set-RdpMultiSession -Enabled

.EXAMPLE
PS C:\> Set-RdpMultiSession -Disabled

.EXAMPLE
PS C:\> Set-RdpMultiSession -Enabled -Force

.EXAMPLE
PS C:\> Set-RdpMultiSession -Enabled -WhatIf
...
function Set-RdpMultiSession {
        ...
        if (-not (Test-IsElevated)) {
            $err = [System.Management.Automation.ErrorRecord]::new(
                [System.UnauthorizedAccessException]::new(
                    'Set-RdpMultiSession requires elevated privileges. ' +
    ...
    process {
        if ($Enabled) {
            ...
            if (-not $PSCmdlet.ShouldProcess('Multi-session policy', 'Disable')) {
                return
            }
